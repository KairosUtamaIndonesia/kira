 ...
 *
 * `parseResponse` validates the untrusted inbound payload and narrows it to the
 * feature's response type. It runs on delivery; a thrown validation error keeps
 * the request pending so the caller can retry with a corrected response.
 */

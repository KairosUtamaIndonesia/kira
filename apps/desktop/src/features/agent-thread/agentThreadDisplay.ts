...
export function buildAgentThreadTranscript(
  ...
  for (let i = 0; i < state.messages.length; i++) {
    const msg = state.messages[i] as TranscriptMessage;
      ...
      if (msg.content && msg.content.length > 0) {
        ...
        for (const block of msg.content) {
          if (block.type === "text") {
            ...
            const status = resolveToolCallStatus(result, block.id, toolOutputs);
...
function resolveToolCallStatus(
  result: TranscriptMessage | undefined,
  toolCallId: string,
  toolOutputs: Record<string, string>,
): ToolCallStatus {
  if (result) return result.isError ? "failed" : "succeeded";
  if (toolOutputs[toolCallId] !== undefined) return "running";
  return "succeeded";
}
...
export function treeEntriesToJson(entries: TreeEntry[]): SessionTreeNodeJson[] {
  ...
  for (const e of entries) {
    const node = map.get(e.id) as SessionTreeNodeJson;
    if (e.parentId && map.has(e.parentId)) {
      (map.get(e.parentId) as SessionTreeNodeJson).children.push(node);
    } else {
      roots.push(node);
    }
  }

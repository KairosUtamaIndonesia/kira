...
const ContextContext = createContext<ContextSchema | undefined>(undefined);
...
export type ContextContentFooterProps = ComponentProps<"div"> & {
  showTotalCost?: boolean | undefined;
};
...
function ContextTotalCost({ value }: { value: string }) {
  return (
    <>
...
const TokensWithCost = ({ tokens, costText }: { tokens?: number; costText?: string }) => (
  <span>
    ...
    {costText ? <span className="ml-2 text-muted-foreground">• {costText}</span> : false}
  </span>
);
...
export const ContextInputUsage = ({ className, children, ...props }: ContextInputUsageProps) => {
  ...
  const inputTokens =
    usage === undefined || usage.inputTokens === undefined ? 0 : usage.inputTokens;
...
export const ContextOutputUsage = ({ className, children, ...props }: ContextOutputUsageProps) => {
  const { usage } = useContextValue();
  const outputTokens =
    usage === undefined || usage.outputTokens === undefined ? 0 : usage.outputTokens;
...
export const ContextReasoningUsage = ({
  ...
  const reasoningTokens =
    usage === undefined || usage.reasoningTokens === undefined ? 0 : usage.reasoningTokens;
...
export const ContextCacheUsage = ({ className, children, ...props }: ContextCacheUsageProps) => {
  const { usage } = useContextValue();
  const cacheTokens =
    usage === undefined || usage.cachedInputTokens === undefined ? 0 : usage.cachedInputTokens;

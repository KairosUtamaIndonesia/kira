...
export async function registerProviderExtensions(): Promise<void> {
  ...
  for (const [providerId, models] of byProvider) {
    ...
    modelRegistry.registerProvider(providerId, {
      ...
      api: (first.api ?? "openai-completions") as string,
      models: models.map((m: CloudModel) => ({
        id: m.upstreamModelId,
        name: m.label,
        reasoning: m.capabilities && m.capabilities.reasoning ? m.capabilities.reasoning : false,
        input:
    ...
    logger.error(`[model-registry] registered ${models.length} ${providerId} models`);
  }

  const def = rawModels.find((m) => m.isDefault);
  if (def) {
    defaultModelRef = { providerId: def.providerId, modelId: def.upstreamModelId };
    logger.error(`[model-registry] default: ${def.providerId}/${def.upstreamModelId}`);
  }
}

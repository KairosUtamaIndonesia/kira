...
export async function registerProviderExtensions(): Promise<void> {
  ...
  try {
    ...
    throw new Error(`Failed to fetch models from cloud: ${(err as Error).message}`);
  }
  ...
  for (const [providerId, models] of byProvider) {
    ...
    modelRegistry.registerProvider(providerId, {
      ...
      api: (first.api ?? "openai-completions") as any,
    ...
    console.error(`[model-registry] registered ${models.length} ${providerId} models`);
  }

  const def = rawModels.find((m) => m.isDefault);
  if (def) {
    defaultModelRef = { providerId: def.providerId, modelId: def.upstreamModelId };
    console.error(`[model-registry] default: ${def.providerId}/${def.upstreamModelId}`);
  }
}

...
const memorySchema = Type.Object({
  ...
  category: Type.Optional(
    StringEnum(
      ["failure", "correction", "insight", "preference", "convention", "tool-quirk"] as const,
      {
...
function createMemoryTool(
  ...
  return {
    ...
    promptGuidelines: [
      ...
      "Do NOT use memory for temporary task state, TODO items, or only for durable, cross-session facts.",
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      const target =
        rawTarget === "project" ? "memory" : (rawTarget as "memory" | "user" | "failure");

...
- delta`,
    ...
    parameters: Type.Object({
      markdown: Type.String({
        description:
          "Markdown request with optional from/to/cwd/limit fields and all/any/exclude lists.",
      }),
    }),
...
Returns conversation snippets with session dates and project context.`,
    ...
    parameters: Type.Object({
      ...
      limit: Type.Optional(
        Type.Number({ description: "Maximum results to return (default: 10, max: 20)." }),
      ),
    }),
    execute: async (
      _id: string,
      args: { query: string; project?: string; role?: string; limit?: number },
    ) => {

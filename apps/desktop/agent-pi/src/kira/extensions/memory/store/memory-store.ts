...
export class MemoryStore {
  ...
  async replace(
    ...
    if (matches.length > 1 && new Set(matches).size > 1) {
      return {
        ...
        matches: matches.map(
          (e) => this.stripMetadata(e).slice(0, 80) + (e.length > 80 ? "..." : ""),
        ),
      };
    }
  ...
  async remove(target: "memory" | "user" | "failure", oldText: string): Promise<MemoryResult> {
    ...
    if (matches.length > 1 && new Set(matches).size > 1) {
      return {
        ...
        matches: matches.map(
          (e) =>
            this.stripMetadata(e).slice(0, 80) + (this.stripMetadata(e).length > 80 ? "..." : ""),
        ),
      };
    }
  ...
  formatForSystemPrompt(): string {
    ...
    if (this.config.failureInjectionEnabled !== false) {
      ...
      const maxFailures =
        this.config.failureInjectionMaxEntries ?? DEFAULT_FAILURE_INJECTION_MAX_ENTRIES;

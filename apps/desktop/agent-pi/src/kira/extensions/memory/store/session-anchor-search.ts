...
export function searchSessionAnchors(
  ...
  if (!fs.existsSync(options.sessionsDir)) {
    return {
      success: false,
      ranges: [],
      message: `sessionsDir does not exist: ${options.sessionsDir}`,
    };
  }
  ...
  for (const file of files) {
    ...
    const fileResult = searchJsonlFile(
      file,
      parsed.request,
      remainingLines,
      scannedLines,
...
function parseMarkdownRequest(
  ...
  for (const line of lines) {
    ...
    if (listMatch && currentList) {
      ...
      if (term.length === 0) {
        return {
          success: false,
          message: `Empty term in '${currentList}'. Remove it or provide text.`,
        };
      }
  ...
  if (from === null)
    return { success: false, message: "Invalid from. Use YYYY-MM-DD or an ISO timestamp." };
  const to = toValue === undefined ? undefined : parseDateTime(toValue, "to");
  if (to === null)
    return { success: false, message: "Invalid to. Use YYYY-MM-DD or an ISO timestamp." };
  if (from && to && from.getTime() > to.getTime()) {
    return {
      success: false,
      message: "Invalid time window. 'from' must be before or equal to 'to'.",
    };
  }
...
function searchJsonlFile(
  ...
  | { success: true; ranges: PendingRange[]; scannedLines: number }
...
function getTimestamp(event: unknown): string | undefined {
  ...
  if (isRecord(event.message) && typeof event.message.timestamp === "string")

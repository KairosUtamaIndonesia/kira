...
function OrganizationHeader({ organization }: OrganizationHeaderProperties) {
  ...
  const activeTab = orgAdminNavigation.find(
  ...
  const breadcrumbItems = useMemo(
    () => [
      ...
      { label: organization.name, href: `/org/${organization.id}` },
      ...(activeTab !== undefined && activeTab.to !== "/org/$organizationId"
  ...
  return (
    <>
      <ConsoleBreadcrumbSetter items={breadcrumbItems} />

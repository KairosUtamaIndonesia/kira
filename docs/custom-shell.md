...
Foundry uses Pi's Bash command tool for Kira's terminal commands. It detects a suitable shell automatically on Windows, macOS, and Linux.
...
3. Select **Test shell**. Foundry runs a harmless command to confirm the executable starts and returns output.
4. Select **Save path**.

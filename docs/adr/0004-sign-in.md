...
Foundry is an internal tool for one company's employees, reached from an Electron
...
Foundry checks `iss` or `tid` before the user row is written. Microsoft is
...
Foundry adds **no check of its own** on the way in. Better Auth's
...
`signIn.social` and then hand the app back. Foundry has no web front end, so the
...
fallback: the user approves at Microsoft's own URL, and no Foundry page is needed
...
Entra is only ever called at sign-in. A long Foundry session therefore does not
...
Entra owns all four. Foundry needs no mail provider, and the auth surface is
...
Foundry does not build: its page would be the server's, and nothing in the desktop
